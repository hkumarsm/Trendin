...
import  com.trendin.core.util.exceptions.POMParseException;
import com.trendin.core.util.fileio.CloseableWorkbook;
import com.trendin.core.util.fileio.FileTools;

/**
 * Encapsulates a file path and sheet name.
 * 
 * This class is used to delay opening a DataDriver object until 
 * the data needs to be read, in order to reduce unnecessary memory usage.
 * 
...
public class DataSource {
	 ...
	 * @param defaultSheet Default sheet name. 
	 * @param src String representing the sheet for the data source. 
	 * Should be of the form "{FilePath}::{SheetName}". If either the file path
	 * or the sheet name is blank, defaultFile and/or defaultSheet are substituted
	 * in respectively. Empty string is treated the same as "::".
	 */
	public DataSource(String defaultPath, String defaultSheet, String src) throws POMParseException
	{
		...
		switch(sheetSplit.length)
		{
			...
			case 2:
				...
				break;
			default:
				throw new POMParseException("Invlaid data source path: " + src);
		}
		...
		switch(ext)
		{
			...
			case CSV:
				break;
			default:
				throw new POMParseException("Invalid file extension: " + sourcePath);
		}
